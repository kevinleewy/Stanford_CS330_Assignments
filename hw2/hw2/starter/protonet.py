...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            # Compute model output for support images
            # torch.Size([num_way * num_support, NUM_HIDDEN_CHANNELS=64])
            out_support = self._network(images_support)

            # Compute the prototypes
            # torch.Size([num_way, NUM_HIDDEN_CHANNELS=64])
            prototypes = torch.stack([out_support[labels_support == y].mean(dim=0) for y in range(labels_support.max() + 1)])

            # Compute distances for support images
            dist_support = []
            for out_i in out_support:
                d = torch.stack([torch.linalg.norm(out_i-prototypes[y]) for y in range(labels_support.max() + 1)])
                dist_support.append(d)
            
            # torch.Size([75, 5])
            dist_support = torch.stack(dist_support)

            # Compute model output for query images
            # torch.Size([num_way * num_query, NUM_HIDDEN_CHANNELS=64])
            out_query = self._network(images_query)

            # Compute distances for query images
            dist_query = []
            for out_i in out_query:
                d = torch.stack([torch.linalg.norm(out_i-prototypes[y]) for y in range(labels_query.max() + 1)])
                dist_query.append(d)
            
            # torch.Size([75, 5])
            dist_query = torch.stack(dist_query)

            # Compute loss
            loss = F.cross_entropy(input=-dist_query, target=labels_query, reduction='mean')

            # Compute accuracies
            accuracy_support = util.score(logits=-dist_support, labels=labels_support)
            accuracy_query = util.score(logits=-dist_query, labels=labels_query)
