...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        batch_size, _, _, dim_input = input_images.size()

        # Zero out query set labels
        input_labels = torch.cat((
            input_labels[:, :self.samples_per_class, :, :],
            torch.zeros_like(input_labels[:, self.samples_per_class:, :, :])
        ), dim=1)

        # Concatenate labels to image
